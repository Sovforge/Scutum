...
### Added
- **Terraform and Pulumi provider** (`contrib/terraform/`, `contrib/pulumi/`): Standalone Terraform provider (`registry.terraform.io/Sovforge/scutum`) built with the Terraform Plugin Framework. Manages five resource types: `scutum_node` (register hub/remote nodes), `scutum_group` (node groups and membership), `scutum_federation_peer` (inter-hub WireGuard tunnels), `scutum_webhook` (webhook endpoint subscriptions), and `scutum_user` (user accounts and role assignment). Authenticates via API key — set `api_key` in the provider block or the `SCUTUM_API_KEY` environment variable. A Pulumi YAML example (`contrib/pulumi/examples/Pulumi.yaml`) shows the equivalent stack using the Pulumi Terraform bridge. Example configurations for both tools are in `contrib/terraform/examples/` and `contrib/pulumi/examples/`.
